...
class MutationGateTests(_LockedConfigMixin, unittest.TestCase):
    ...
    def test_locked_dashboard_rejects_agents_post_without_token(self):
        ...
        server.Handler._h_agents_post(
            fake, urlparse("/api/agents"), {"agent": {"name": "x"}})
        self.assertEqual(fake.status, 403)
    ...
    def test_hooks_post_gated(self):
        fake = _FakeHandler(headers={})
        server.Handler._h_agent_hooks_post(
            fake, urlparse("/api/agent-hooks"), {"hooks": {}})
        self.assertEqual(fake.status, 403)

    def test_workflows_post_gated(self):
        fake = _FakeHandler(headers={})
        server.Handler._h_agent_workflows_post(
            fake, urlparse("/api/agent-workflows"), {"config": {}})
        self.assertEqual(fake.status, 403)
    ...
    def test_agents_remove_gated(self):
        fake = _FakeHandler(headers={})
        server.Handler._h_agents_remove(
            fake, urlparse("/api/agents/remove"), {"name": "x"})
        self.assertEqual(fake.status, 403)
...
class UnlockedDashboardTests(unittest.TestCase):
    """When no lock is configured, mutations proceed without a token."""
