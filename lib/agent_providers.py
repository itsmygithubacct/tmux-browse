...
def _text_from_vendor_content(content: Any) -> str:
    """vendor messages: content is a list of content blocks with type 'text'."""
    if isinstance(content, list):
        parts: list[str] = []
        for item in content:
...
def _vendor_messages_url(base_url: str) -> str:
...
def vendor_messages(agent: dict[str, Any], messages: list[dict[str, str]],
                       timeout: float) -> str:
    base_url = agent["base_url"].rstrip("/")
    ...
    for message in messages:
        ...
        if role not in {"user", "assistant"}:
            raise UsageError(f"unsupported vendor message role: {role}")
    ...
    headers = {
        ...
        "vendor-version": "2023-06-01",
        "Content-Type": "application/json",
    }
    data = _post_json(_vendor_messages_url(base_url), headers, payload, timeout=timeout)
    try:
        return _text_from_vendor_content(data["content"])
...
PROVIDERS: dict[str, ProviderFn] = {
    ...
    "vendor-messages": vendor_messages,
}
