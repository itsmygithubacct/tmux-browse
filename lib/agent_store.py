...
SUPPORTED_WIRE_APIS = {"openai-chat", "vendor-messages"}
...
_BUILTIN_CATALOG: dict[str, dict[str, str]] = {
    "sonnet": {
        "label": "assistant Sonnet",
        "provider": "vendor",
        "model": "assistant-sonnet-4-6",
        "base_url": "https://api.vendor.com/v1",
        "wire_api": "vendor-messages",
    },
    "opus": {
        "label": "assistant Opus",
        "provider": "vendor",
        "model": "assistant-opus-4-7",
        "base_url": "https://api.vendor.com/v1",
        "wire_api": "vendor-messages",
    },
    "gpt": {
...
def _normalize_agent_meta(name: str, meta: dict[str, Any]) -> dict[str, Any]:
    ...
    # Older built-in vendor entries were stored as openai-chat; transparently
    # migrate them so existing users do not have to re-add the agent.
    if provider == "vendor" and base_url == "https://api.vendor.com/v1" and wire_api == "openai-chat":
        wire_api = "vendor-messages"
...
def _apply_builtin_constraints(name: str, entry: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "kimi":
        ...
        if out.get("wire_api") != "vendor-messages":
            raise UsageError("kimi must use wire API 'vendor-messages'")
        out["provider"] = "kimi"
    return out
