...
- ``POST /api/peers/pair-request`` — incoming: a peer is asking to
  ...
  accept/decline UI.
- ``POST /api/peers/pair-accept`` — outgoing initiator OR incoming
  callback. Body ``{device_id}`` accepts a previously-pending
  incoming request and writes the pair to our store; on initiator
  side, the same handler is used for the peer's confirmation that
  *they* accepted us.
...
- ``POST /api/peers/unpair`` — operator action: remove a peer
  from our paired set (the other side is informed, but its
  pairing state is its own concern; documented in
  docs/federation.md).

Pair-request/accept handlers don't require the config-lock token
because the request is itself a network operation that the
operator hasn't yet authenticated. The operator-action handlers
(pair-request-out, pair-decline, unpair) DO require it because
...
def h_unpair(handler: "Handler", _parsed: ParseResult, body: dict) -> None:
    """Operator: remove a peer from the paired set. We don't notify
    the peer — their pairing state is theirs. Documented in
    docs/federation.md."""
    if not handler._check_unlock():
        return
    did = (body.get("device_id") or "").strip()
