...
By default they map to assistant Sonnet 4.6, assistant Opus 4.7, `gpt-5.4`,
...
printf '%s' "$vendor_API_KEY" | tb agent add sonnet --api-key-stdin
printf '%s' "$vendor_API_KEY" | tb agent add opus --api-key-stdin
...
`openai-chat` and `vendor-messages`.
