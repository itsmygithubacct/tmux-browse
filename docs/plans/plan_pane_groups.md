...
- Creating an "Agents" group and moving the assistant_code pane into it
  renders a furled `Agents (1)` `<details>` with assistant_code inside,
  and assistant_code no longer appears under Visible.
- Removing the Agents group sends assistant_code back to Visible with its
  order preserved.
